...
class SupertrendScript(ScriptStrategyBase):
    """
    The strategy is to buy and sell on trend change
    """
    ...
    # DEPRECATED DEPRECATED DEPRECATED DEPRECATED rsi_length = int(os.getenv("RSI_LENGTH", "14"))
    atrPeriod = int(os.getenv("ATR_PERIOD", "10"))
    supertrendFactor = int(os.getenv("FACTOR", "3"))
    ...
    # DEPRECATED DEPRECATED DEPRECATED DEPRECATED buy_rsi = int(os.getenv("BUY_RSI", "30"))
    # DEPRECATED DEPRECATED DEPRECATED DEPRECATED sell_rsi = int(os.getenv("SELL_RSI", "70"))
    ...
    def on_tick(self):
        """
        On every tick calculate OHLCV candlesticks, calculate Supertrend, react on trend change signal with creating,
        ...
        if not self.subscribed_to_order_book_trade_event:
            ...
            df = self.calculate_supertrend(df, self.atrPeriod, self.supertrendFactor)
            ...
            if should_open_position or should_close_position:
                ...
                self._direction = df.iloc[-1]['direction']
                self.logger().info(f"Direction is {self._direction:.0f}")
    ...
    def should_open_position(self, df: pd.DataFrame) -> bool:
        ...
        superTrend_is_calculated = pd.notna(direction)
        time_to_buy = superTrend_is_calculated and direction < 0
    ...
    def should_close_position(self, df: pd.DataFrame) -> bool:
        ...
        direction: int = df.iloc[-1]['direction']
        superTrend_is_calculated = pd.notna(direction)
        time_to_sell = superTrend_is_calculated and direction > 0
    ...
    def send_order(self, order: OrderCandidate):
        ...
        if is_buy:
            msg = f"Supertrend is bull {self.buy_rsi:.2f}, buying {order.amount:.5f} {self.base} with limit order at {order.price:.2f} ."
        else:
            msg = (f"Supertrend is bear {self.sell_rsi:.2f}, selling {self.position.amount:.5f} {self.base}"
    ...
    @staticmethod
    def calculate_supertrend(df: pd.DataFrame, atrPeriod: int = 10, factor: int = 3):
        """
        Calculate relative strength index and add it to the dataframe.
        """
        close_delta = df['close'].diff()
        ...
        atr = true_range.rolling(atrPeriod).sum()/atrPeriod

        src = (df['high'] + df['high'])/2
        #atr = ta.atr(atrPeriod)
        upperBand = src + factor * atr
        lowerBand = src - factor * atr
        prevLowerBand = df['lowerBand'].shift(1).fillna(method='bfill')
        prevUpperBand = df['upperBand'].shift(1).fillna(method='bfill')

        if lowerBand > prevLowerBand or df['close'].shift(1) < prevLowerBand
            lowerBand = lowerBand
        else
            lowerBand = prevLowerBand
        
        if upperBand < prevUpperBand or df['close'].shift(1) > prevUpperBand
            upperBand = upperBand
        else
            upperBand = prevUpperBand

        int direction = None
        float superTrend = None
        prevSuperTrend = df['superTrend'].shift(1)

        if df['atr'].shift(1) is None
            direction := 1
        else if prevSuperTrend == prevUpperBand
            if df['close'] > upperBand
                direction = -1
            else
                direction = 1
        else
            if df['close'] < lowerBand
                direction = 1
            else
                direction = -1
    ...
    def format_status(self) -> str:
        ...
        lines.extend(["", "  Current Direction:"] + ["    " + f"{self._supertrend:.0f}"])
        lines.extend(["", "  Simple Supertrend strategy total price change with all trades:"] + ["    " + f"{self._cumulative_price_change_pct:.5f}" + " %"])
